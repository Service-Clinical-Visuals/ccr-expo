...
const NAV_ITEMS: NavItem[] = [
  ...
  { label: "Repair Of TransDucers", href: "#" },
  { label: "EU Projects", href: "#" },
  { label: "FFP2 Masks", href: "#" },
  { label: "OEM", href: "#" },
  { label: "Soluction", href: "#" },
  { label: "Industrial Activity", href: "#" },
  { label: "News", href: "#" },
];
    ...
    >
      <div className="w-full px-4 sm:px-6 md:px-8 xl:px-35 mx-auto">
        <div className="flex items-center justify-between py-3 sm:py-3 transition-all duration-300 ease-in-out">
          ...
          <nav className="hidden xl:flex items-center gap-3 xl:gap-6 font-inter text-sm">
            {NAV_ITEMS.map((item) => {
                  ...
                  <div className="flex items-center gap-1 cursor-pointer py-1">
                    <Link
                      ...
                      className={`relative header-link font-inter transition-colors duration-200 ${isHome
                        ? "text-[#4B5563] font-semibold after:absolute after:-bottom-0.5 after:left-0 after:w-full after:h-[2px] after:bg-slate-500"
                        : "text-slate-600 hover:text-[#4B5563] font-medium"
                        }`}
                    >
                      {item.label}
                    </Link>
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            ...
            <div className="hidden sm:block">
              <Button href="#language" variant="primary" showArrow={false} className="!bg-[#0287DC] hover:!bg-[#0275bd] !rounded-[4px] !w-auto border-none">
                <span className="flex items-center gap-2 font-inter font-bold btn-text text-white ">Language <ChevronDown className="w-4 h-4" /></span>
              </Button>
            </div>
            ...
            <nav className="flex flex-col gap-1 font-inter">
              ...
              <div className="pt-4 mt-2 border-t border-slate-100 sm:hidden flex justify-center">
                <Button
                  href="#language"
                  variant="primary"
                  className="w-full justify-center !bg-[#0287DC] hover:!bg-[#0275bd] !rounded-[4px] font-inter border-none"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  <span className="flex items-center gap-2 font-inter text-white">Language <ChevronDown className="w-4 h-4" /></span>
                </Button>
              </div>
            </nav>
          </div>
