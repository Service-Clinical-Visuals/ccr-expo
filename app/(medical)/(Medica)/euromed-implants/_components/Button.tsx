...
const CustomArrow = ({ className }: { className?: string }) => (
  <svg width="17" height="15" viewBox="0 0 15 11" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const Button = ({
  ...
  const content = (
    <div className={`flex items-center justify-center px-6 py-2.5 rounded-xl transition-colors ${variantStyles} ${className}`}>
      <span className={`button whitespace-nowrap ${showIcon ? "mr-2" : ""}`}>{text}</span>
      {showIcon && (
