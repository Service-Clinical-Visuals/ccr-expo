...
const CustomArrow = ({ className }: { className?: string }) => (
  <svg width="17" height="17" viewBox="0 0 15 11" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
...
const Button = ({
  ...
  const content = (
    <div className={`button flex items-center justify-center px-8 py-3.5 transition-colors ${variantStyles} ${className}`}>
      <span className="whitespace-nowrap mr-3 sm:mr-4">{text}</span>
      {showIcon && (
        icon ? icon : <CustomArrow className="group-hover:translate-x-1 transition-transform duration-300 shrink-0" />
      )}
    </div>
  );
